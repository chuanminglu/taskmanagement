...
> **📤 输出产物**：数据表结构 + 索引 + ER图
> **🔗 下游使用**：开发团队建表、API开发、测试用例设计

基于以下输入材料，设计完整的数据库Schema（表结构+索引+关系）。
...
适用场景：已完成Feature拆解和用户故事生成，需要为整个Feature设计数据库
...
业务价值：访客通过微信小程序提前预约，减少现场等待时间

用户故事清单：
Story #1: 作为访客，我希望填写预约表单（姓名、手机号、身份证号、访问事由），以便提交访客信息
Story #2: 作为访客，我希望从通讯录选择被访人，以便准确定位联系人
Story #3: 作为访客，我希望选择访问日期和时段，以便提前安排行程
Story #4: 作为访客，我希望上传身份证照片（可选），以便加速审批
Story #5: 作为访客，我希望查看预约状态（审批中/已通过/已拒绝），以便了解审批进度
Story #6: 作为被访人，我希望收到审批通知，以便及时处理访客申请
Story #7: 作为被访人，我希望查看访客详细信息，以便决定是否批准
Story #8: 作为被访人，我希望批准或拒绝预约，以便控制访客准入
```
...
适用场景：聚焦单个Story或相关Story组，进行精细化设计
...
**规则1：识别实体（从Story的"我希望"部分）**

| Story关键词          | 实体类型   | 表设计                                    |
| -------------------- | ---------- | ----------------------------------------- |
| 填写表单、提交信息   | 业务主实体 | 创建主表（如{主业务}表、orders）        |
| 选择{关联对象}、选择商品 | 关联实体   | 外键关联或关联表（如{关联对象}表、products） |
| 查看状态、查看列表   | 无需新表   | 在主表增加status字段或查询接口            |
| 上传文件、上传图片   | 附件实体   | 创建附件表（如attachments）               |
| 审批、批准、拒绝     | 审批流程   | 增加审批相关字段或审批记录表              |

**规则2：识别字段（从Story的验收标准）**
...
**规则3：识别关系（从Story之间的引用）**

```
Story #1: 作为{角色A}，我希望提交{业务主实体}
Story #6: 作为{角色B}，我希望查看{业务主实体}详细信息
↓
关系识别：
- {主表}（主业务记录）1:N {从表1}（详细信息）
- {主表} N:1 {从表2}（关联对象）
```
...
| 实体名称 | 说明       | 关键属性   | 数量级   | 业务规则   | 来源Story  |
| -------- | ---------- | ---------- | -------- | ---------- | ---------- |
| {实体1}  | {功能说明} | {核心字段} | {数据量} | {特殊规则} | Story #{N} |
| {实体2}  | {功能说明} | {核心字段} | {数据量} | {特殊规则} | Story #{N} |

**示例（基于访客预约Feature + 8个Story）**：

| 实体名称                       | 说明                 | 关键属性                              | 数量级     | 业务规则                           | 来源Story                    |
| ------------------------------ | -------------------- | ------------------------------------- | ---------- | ---------------------------------- | ---------------------------- |
| 访客预约（Reservation）        | 访客提交的预约申请   | 预约ID、访客ID、被访人ID、预约时间    | 日增~50条  | 预约时间不能早于当前时间           | Story #1（访客发起预约申请） |
| 访客信息（Visitor）            | 访客基本信息         | 访客ID、姓名、手机号、身份证号        | 月增~200条 | 身份证号唯一、手机号唯一           | Story #1（填写访客信息）     |
| 被访人（Employee）             | 园区员工信息         | 员工ID、姓名、部门、手机号            | ~500条     | 员工ID唯一                         | Story #2（被访人审批）       |
| 审批记录（ApprovalRecord）     | 被访人对预约的审批   | 记录ID、预约ID、审批结果、审批时间    | 日增~50条  | 一个预约只能有一条最终审批记录     | Story #2（被访人审批预约）   |
| 门禁核验记录（AccessLog）      | 访客进出园区的记录   | 记录ID、预约ID、核验时间、核验设备    | 日增~100条 | 同一预约可有多次进出记录           | Story #4（安保门禁核验）     |
| 访客反馈（VisitorFeedback）    | 访客对服务的评价反馈 | 反馈ID、预约ID、评分（1-5）、反馈内容 | 周增~30条  | 一个预约可有0或1条反馈             | Story #6（访客提交反馈）     |
| 附件（Attachment）             | 身份证照片等附件     | 附件ID、预约ID、文件URL、文件类型     | 日增~50条  | 支持多个附件（身份证正反面）       | Story #1（上传身份证照片）   |
| 访问数据统计（VisitStatistic） | 园区管理员统计数据   | 统计ID、统计日期、预约数、访客数      | 月增~30条  | 按月/周/日三种粒度统计（冗余设计） | Story #8（访问数据报表）     |
